        ...
        for (int j=0; j<m; j++) {
            ...
            if (visited.find(tmps) != visited.end()) {
                ...
                if (g_stat_cache.find(curres_vec) != g_stat_cache.end()) {
                    // 缓存命中 - 直接使用缓存的结果
                    pair<int, int> cached_result = g_stat_cache[curres_vec];
                    neighbor_bdwt = cached_result.first;
                    neighbor_load = cached_result.second;
                    //cout << "        [CACHE HIT] load: " << neighbor_load << ", bdwt: " << neighbor_bdwt << endl;
                } else {
                    // 缓存未命中 - 计算结果并缓存
                    stat(sidx2ip, curres_vec, itm_idx, ecdag, &neighbor_bdwt, &neighbor_load);
                    // 将结果存入缓存
                    g_stat_cache[curres_vec] = make_pair(neighbor_bdwt, neighbor_load);
...
Solution* genTradeoffCurve(vector<int> itm_idx, vector<int> candidates,
    ...
    // ========== 缓存优化：在首次调用时清空缓存 ==========
    // 注意：同一个 ECDAG 内的多轮搜索可以复用缓存
    // 如果需要在新的 ECDAG 时清空，需要在 main() 中显式调用 clearStatCache()
    // ============================================
